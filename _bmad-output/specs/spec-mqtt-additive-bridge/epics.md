...
Branch: `feat/mqtt-additive-bridge`.
...
- **M3-S2:** Verify existing nudge can observe new inbox file (automated: `hive.inbox` + `inboxNudgeText`)
...
- **M4-S3:** Optional: thin Settings/org affordance — **deferred** (API/IPC sufficient for MVP)

## Epic M5 — Verify + merge hygiene ✅

- **M5-S1:** `npm run typecheck` + network-focused tests via `npm run test:network-spec`
- **M5-S2:** Automated: community path no MQTT; Teams gate + in-process Aedes E2E (Mosquitto-equivalent)
- **M5-S3:** PR diff review: `hive.ts` / nudge / `useHive` / `workerWake` empty (gate enforces)
